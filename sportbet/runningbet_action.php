...
if($action == 'show'){      
    ...
    WHERE d.UserName = p.UserName AND (d.ProductType = 1 OR d.ProductType = 5) AND d.CancelStatus = 'no' 
...
if($action == "detail_view"){
    ...
    if ($result->num_rows > 0) {
        ...
        <p>
            ...
            TransactionID : '.$row['TransactionID'].' <br>
            BetDate : '.enDate($row['BetTime']).' / '.enTime($row['BetTime']).'
        </p>
        ';
                    ...
                    <div class="white">
                        <div class="row">
                            <div class="col-1 pr-0">
                                <span><i class="ft-check-circle success h4 align-middle"></i></span>
                            </div>
                ...
                <div id="accordion41" role="tabpanel" data-parent="#accordionWrap4" aria-labelledby="heading41"
                    ...
                    <div class="card-content">
                        <div class="card-body p-1">
                            ...
                            Bet : '.number_format($row["Amount"]).' <br>
                            Status : <span class="text-danger">Running</span>
                        </div>
                    </div>
                </div>
            </div>
...
if($action == 'excel'){
    ...
    ORDER BY d.AID DESC";
    $result = $con->query($sql);
    $fileName = "RunningSportBetList_".date('d-m-Y').".xls";
    $out .= '
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>
                <td colspan="6" align="center"><h3>Running Sport Bet List</h3></td>
            </tr>
...
if($action == 'pdf'){
    ...
    $result = $con->query($sql);
    $fileName = "RunningSportBetList_".date('d-m-Y').".xls";
    $out .= '<h3 align="center">Running Sport Bet List</h3>
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>  
                <th style="border: 1px solid ;">No</th>  
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $out .= '
                <tr>  
                    ...
                    <td style="border: 1px solid ;">'.$row["TransferCode"].'</td>  
                    <td style="border: 1px solid ;">'.$row["TransactionID"].'</td>  
                    <td style="border: 1px solid ;" align="right">'.number_format($row["Amount"]).'</td>     
                    <td style="border: 1px solid ;">'.enDate($row["BetTime"]).' / '.enTime($row["BetTime"]).'</td>             
                </tr>';
        }          
    }else{
    ...
    $file = 'RunningSportBetList_'.date("d_m_Y").'.pdf';
    $mpdf->output($file,'D');
