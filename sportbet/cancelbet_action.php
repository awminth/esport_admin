...
if($action == 'show'){      
    ...
    $out = '';
    $sql = "SELECT d.*,p.AID as player_id FROM tblcancelbet d,tblplayer p   
    WHERE d.UserName = p.UserName AND (d.ProductType = 1 OR d.ProductType = 5) ".$a." 
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $out .= '
                <tr>
                    ...
                    <td class="text-right text-danger">'.number_format($bet_amt).'</td>
        ...
        $sql_total = "SELECT COUNT(d.AID) AS total FROM tblcancelbet d,tblplayer p
        WHERE d.UserName = p.UserName AND (d.ProductType = 1 OR d.ProductType = 5) " .$a." 
        ORDER BY d.AID DESC";
...
if($action == "detail_view"){
    ...
    $out = '';
    $sql = "SELECT d.* FROM tblcancelbet d  
    ...
    if ($result->num_rows > 0) {
        ...
        // }
        // after label
        $label = $body_data["betOption"];
        if($body_data["sportType"] == "Mix Parlay" || $body_data["sportType"] == "MixParlay"){
            $label = $body_data["sportType"];
        }
        // title header
        $title_str = $body_data["league"];
        if(strlen($body_data["league"]) > 32){
            $title_str = substr($body_data["league"], 0, 32).' ...';
        }
        $out.='
                    ...
                    <div class="white">
                        <div class="row">
                            <div class="col-1 pr-0">
                                <span><i class="ft-minus-circle warning h4 align-middle"></i></span>
                            </div>
                ...
                <div id="accordion41" role="tabpanel" data-parent="#accordionWrap4" aria-labelledby="heading41"
                    ...
                    <div class="card-content">
                        <div class="card-body p-1">
                            ...
                            Bet Type : <span class="text-info">'.$body_data["marketType"].'</span> <br>                            
                            League : '.$body_data["league"].' <br>
                            Match : '.$body_data["match"].' <br>
                            Bet Option :  <span class="text-warning">'.$body_data["betOption"].'</span> <br>
                            Bet : '.number_format($bet_amt).' <br>
                            Status : <span class="text-danger">Cancel</span>
                        </div>
                    </div>
                </div>
            </div>
...
if($action == 'excel'){
    ...
    ORDER BY d.AID DESC";
    $result = $con->query($sql);
    $fileName = "CancelSportBetList_".date('d-m-Y').".xls";
    $out .= '
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>
                <td colspan="6" align="center"><h3>Cancel Sport Bet List</h3></td>
            </tr>
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $no = $no + 1;
            $bet_amt = GetInt("SELECT Amount FROM tbldeduct WHERE TransferCode = ? ",[$row["TransferCode"]]);
            $bet_dt = GetInt("SELECT BetTime FROM tbldeduct WHERE TransferCode = ? ",[$row["TransferCode"]]);
            $out .= '
                <tr>  
                    <td style="border: 1px solid ;">'.$no.'</td>  
                    <td style="border: 1px solid ;">'.$row["UserName"].'</td>  
                    <td style="border: 1px solid ;">'.$row["TransferCode"].'</td>  
                    <td style="border: 1px solid ;">'.$row["TransactionID"].'</td>  
                    <td style="border: 1px solid ;">'.number_format($bet_amt).'</td>       
                    <td style="border: 1px solid ;">'.enDate($bet_dt).' / '.enTime($bet_dt).'</td>             
                </tr>';
        }          
    }else{
        $out .= '
...
if($action == 'pdf'){
    ...
    ORDER BY d.AID DESC";
    $result = $con->query($sql);
    $out .= '<h3 align="center">Cancel Sport Bet List</h3>
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>  
                <th style="border: 1px solid ;">No</th>  
    ...
    $file = 'CancelSportBetList_'.date("d_m_Y").'.pdf';
    $mpdf->output($file,'D');
