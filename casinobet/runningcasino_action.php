...
if($action == 'show'){      
    ...
    WHERE d.UserName = p.UserName AND d.ProductType = 9 AND d.CancelStatus = 'no' 
...
if($action == 'excel'){
    ...
    ORDER BY d.AID DESC";
    $result = $con->query($sql);
    $fileName = "RunningCasinoBetList_".date('d-m-Y').".xls";
    $out .= '
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>
                <td colspan="6" align="center"><h3>Running Casino Bet List</h3></td>
            </tr>
...
if($action == 'pdf'){
    ...
    $file = 'RunningCasinoBetList_'.date("d_m_Y").'.pdf';
    $mpdf->output($file,'D');
