...
if($action == 'show'){      
    ...
    $out = '';
    $sql = "SELECT l.*,u.UserName FROM tblnotification l,tbluser u 
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if(strlen($row["Title"]) > 28){
                $title_str = substr($row["Title"], 0, 28).' ...';
            }
            $txt_color = $row["Status"] == 0 ? '' : 'text-danger';
            $out .= '            
            <div class="col-sm-6">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title '.$txt_color.'">
                        ...
                        <div class="heading-elements">
                            ...
                            if($row["Status"] == 0 ){
                                ...
                                <a class="text-success" href="#" 
                                    id="btnactive" 
                                    data-aid="'.$row["AID"].'" ><i class="la la-microphone"></i></a>';
                            }else{
                                $out .= '<a class="text-success" href="#" 
                                    id="btnunactive" 
                                    data-aid="'.$row["AID"].'" ><i class="la la-microphone-slash"></i></a>';
                            }   
                            $out .= '
                        </div>
                    </div>
        ...
        FROM tblnotification l,tbluser u 
...
if($action == "save"){
    ...
    try{
        $sql = "INSERT INTO tblnotification (Title, Description, DT, UserID) 
        VALUES (?, ?, ?, ?)";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("sssi", $title, $desc, $dt, $userid);
        if($stmt->execute()){
            mysqli_commit($con);
            save_log($_SESSION["esport_admin_username"]." သည် Notification အား အသစ် သွင်းသွားသည်။");
            echo 1;
        }else{
           mysqli_rollback($con);
           error_log("Save error in notification action, ".$stmt->error."\n", 3, root."others/my_log_file.log");
           echo 0; 
        }
...
if($action == "edit"){
    ...
    mysqli_begin_transaction($con);
    try{
        $sql = "UPDATE tblnotification SET Title=?, Description=?, DT=?, 
        UserID=? WHERE AID=?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("sssii", $title, $desc, $dt, $userid, $aid);
        if($stmt->execute()){
            mysqli_commit($con);
            save_log($_SESSION["esport_admin_username"]." သည် Notification အား edit သွားသည်။");
            echo 1;
        }else{
           mysqli_rollback($con);
           error_log("Edit error in notification action, ".$stmt->error."\n", 3, root."others/my_log_file.log");
           echo 0; 
        }
    }catch(mysqli_sql_exception $e){
        mysqli_rollback($con);
...
if($action == "delete"){
    ...
    mysqli_begin_transaction($con);
    try{
        $sql = "DELETE FROM tblnotification WHERE AID = ?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("i", $aid);
        if($stmt->execute()){
            mysqli_commit($con);
            save_log($_SESSION["esport_admin_username"]." သည် Notification အား delete သွားသည်။");
            echo 1;
        }else{
            mysqli_rollback($con);
            error_log("Delete error in notification action, ".$stmt->error."\n", 3, root."others/my_log_file.log");
            echo 0; 
        }
    }catch(mysqli_sql_exception $e){
        mysqli_rollback($con);
...
if($action == "active"){
    ...
    mysqli_begin_transaction($con);
    try{
        $sql = "UPDATE tblnotification SET Status=1 WHERE AID=?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("i", $aid);
        if($stmt->execute()){
            mysqli_commit($con);
            save_log($_SESSION["esport_admin_username"]." သည် Notification အား unactive လုပ်သွားသည်။");
            echo 1;
        }else{
           mysqli_rollback($con);
           error_log("Edit error in notification action, ".$stmt->error."\n", 3, root."others/my_log_file.log");
...
if($action == "unactive"){
    ...
    mysqli_begin_transaction($con);
    try{
        $sql = "UPDATE tblnotification SET Status=0 WHERE AID=?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("i", $aid);
        if($stmt->execute()){
            mysqli_commit($con);
            save_log($_SESSION["esport_admin_username"]." သည် Notification အား active လုပ်သွားသည်။");
            echo 1;
        }else{
           mysqli_rollback($con);
           error_log("Edit error in notification action, ".$stmt->error."\n", 3, root."others/my_log_file.log");
