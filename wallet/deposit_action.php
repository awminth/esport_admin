...
if($action == 'show'){      
    ...
    WHERE b.PlayerID=p.AID AND b.PrepaidStatus='prepaid' AND b.WinLossStatus='deposit' ";
    $a = '';
    if (!empty($search)) {
...
if($action == 'save'){
    ...
    try{
        $sql = "INSERT INTO tblbalancein (PlayerID, Amount, TransitionCode, PayType, 
        ...
        if($stmt->execute()){
            ...
            save_log($_SESSION["esport_admin_username"]." သည် [".$playername."] ၏ top-up [Amt : ".$amount."] အား ဖြည့်သွင်းသွားသည်။");

            mysqli_commit($con);
            echo 1;
        }else{
            mysqli_rollback($con);
            error_log("Database error in top-up by admin ".$stmt->error."\n", 3, root."wallet/my_log_file.log");
            echo 0;
        }
    }catch (mysqli_sql_exception $e) {
        // Rollback on any error
        mysqli_rollback($con);
        error_log("Database error in top-up by admin, " . $e->getMessage(), 3, root."wallet/my_log_file.log");
        echo 0;
    } catch (Exception $e) {
        // Rollback on any other error
        mysqli_rollback($con);
        error_log("External error in top-up by admin, " . $e->getMessage(), 3, root."wallet/my_log_file.log");
        echo 0;
    }
}

// ငွေဖြည့်ခြင်း အတည်ပြု success ပေး
if($action == 'success'){
    $aid = $_POST['aid'];
    $playerid = $_POST['playerid'];
    ...
    try{
        ...
        if($stmt->execute()){
            ...
            // save log
            save_log($_SESSION["esport_admin_username"]." သည် [".$playername."] ၏ top-up [Amt : ".$amount."] အား Confirm သွားသည်။");

            mysqli_commit($con);
            echo 1;
        }else{
            mysqli_rollback($con);
            error_log("Update data error in deposit success ".$stmt->error."\n", 3, root."wallet/my_log_file.log");
            echo 0;
        }
    }catch (mysqli_sql_exception $e) {
        // Rollback on any error
        mysqli_rollback($con);
        error_log("Database error in deposit success, " . $e->getMessage(), 3, root."wallet/my_log_file.log");
        echo 0;
    } catch (Exception $e) {
        // Rollback on any other error
        mysqli_rollback($con);
        error_log("External error in deposit success, " . $e->getMessage(), 3, root."wallet/my_log_file.log");
        echo 0;
    }
}

// ငွေဖြည့်ခြင်း ပယ်ဖျက် fail ပေး
if($action == 'fail'){
    $aid = $_POST['aid'];
    $playerid = $_POST['playerid'];
    $playername = $_POST['playername'];
    ...
    mysqli_begin_transaction($con);
    try{
        $sql = "UPDATE tblbalancein SET PrepaidStatus='fail' WHERE AID = ?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param("i", $aid);
        if($stmt->execute()){
            // save log
            save_log($_SESSION["esport_admin_username"]." သည် [".$playername."] ၏ top-up [Amt : ".$amount."] အား Cancel သွားသည်။");

            mysqli_commit($con);
            echo 1;
        }else{
            mysqli_rollback($con);
            error_log("Update data error in deposit fail  ".$stmt->error."\n", 3, root."wallet/my_log_file.log");
            echo 0;
        }
    }catch (mysqli_sql_exception $e) {
        // Rollback on any error
        mysqli_rollback($con);        
        error_log("Database error in deposit fail, " . $e->getMessage(), 3, root."wallet/my_log_file.log");
        echo 0;
    } catch (Exception $e) {
        // Rollback on any other error
        mysqli_rollback($con);        
        error_log("External error in deposit fail, " . $e->getMessage(), 3, root."wallet/my_log_file.log");
        echo 0;
    }
}
...
if($action == 'excel'){
    ...
    $result = $con->query($sql);
    $fileName = "PlayerTopup_".date('d-m-Y').".xls";
    $out .= '<head><meta charset="UTF-8"></head>
        <table >  
            <tr>
                <td colspan="7" align="center"><h3>Player Topup</h3></td>
            </tr>
...
if($action == 'pdf'){
    ...
    $result = $con->query($sql);
    $fileName = "PlayerTopup_".date('d-m-Y').".xls";
    $out .= '<h3 align="center">Player Topup</h3>
    ...
    $file = 'PlayerTopup_'.date("d_m_Y").'.pdf';
    $mpdf->output($file,'D');
