...
if($action == 'show'){      
    ...
    $to = $_POST['dtto'];
    if($from != "" || $to != ""){
        $a .= " AND Date(d.ResultTime)>='{$from}' AND Date(d.ResultTime)<='{$to}' ";
    } 
    // player search
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $out .= '
                <tr>
                    ...
                    <td class="" data-format="text">'.$row['TransferCode'].'</td>
...
if($action == 'excel'){
    ...
    ORDER BY d.AID DESC";
    $result = $con->query($sql);
    $fileName = "SettledReportList_".date('d-m-Y').".xls";
    $out .= '
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>
                <td colspan="6" align="center"><h3>Settled Report List</h3></td>
            </tr>
...
if($action == 'pdf'){
    ...
    $result = $con->query($sql);
    $fileName = "SettledReportList_".date('d-m-Y').".xls";
    $out .= '<h3 align="center">Settled Report List</h3>
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>  
                <th style="border: 1px solid ;">No</th>  
    ...
    $file = 'SettledReportList_'.date("d_m_Y").'.pdf';
    $mpdf->output($file,'D');
