        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $out .= '
                <tr>
                    ...
                    <td class="text-danger text-right">'.number_format($bet_amt).'</td>
                    <td class="">'.enDate($bet_dt).' / '.enTime($bet_dt).'</td>
                </tr>';
            $no++;
        }
        $out.="</tbody>";
...
if($action == 'excel'){
    ...
    ORDER BY d.AID DESC";
    $result = $con->query($sql);
    $fileName = "CancelReportList_".date('d-m-Y').".xls";
    $out .= '
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>
                <td colspan="6" align="center"><h3>Cancel Report List</h3></td>
            </tr>
...
if($action == 'pdf'){
    ...
    $result = $con->query($sql);
    $fileName = "CancelReportList_".date('d-m-Y').".xls";
    $out .= '<h3 align="center">Cancel Report List</h3>
        <head><meta charset="UTF-8"></head>
        <table >  
            <tr>  
                <th style="border: 1px solid ;">No</th>  
    ...
    $file = 'CancelReportList_'.date("d_m_Y").'.pdf';
    $mpdf->output($file,'D');
